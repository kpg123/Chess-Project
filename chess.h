...
#define WKING_START_POS 252/* TODO: Starting position for the white king */
...
#define BOARD(x) player[x].p | player[x].r | player[x].n | player[x].b | player[x].q | player[x].k	/* TODO: given a player x, this macro evaluates to the player's board, i.e., 1 in dicates presense of any piece of the same color on the board, 0 indicates absense */
#define FULL_BOARD		(BOARD(player[WHITE]) | BOARD(player[BLACK]))   /* Full board comprising of both players */
#define OCCUPIED(n) IS_SET(FULL_BOARD, pos)		/* TODO: Macro to tell if a square is occupied by a piece of any color */
...
/*Bool to tell whether or not an ep_square is present*/
extern Bool noEp;
...
void clear(Pos pos,Piece type,PlayerColor color);
void restore(Pos pos,Piece type,PlayerColor color);
